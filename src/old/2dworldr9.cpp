...
//MOUSE SUPPORT!!!! now you can select objects with mouse and send them to rally points with right click
//added cursor class
//added mouse_function to object class
//added left_clicked() and right_clicked() booleans to the object class
//added selected to object class
//added rally to object class
//radius is now calculated in the object constructor
//added within_radius to the object class
//added a condition to collision_detection which measures the proximity between objects
//changed to orthographic view
//added mouse_click method and mouse_drag method
//text now doesn't display for some reason
//changed mouse_func to mouse_click
//changed mouse to mouse_drag
//changed object width,height and step size to work better in ortho mode
//added initializeWindow
//changed the changesize method for ortho
//modified the main method to include the mouse methods and work in ortho mode
...
void collision_detection()
{
    for(int a=0; a<max_objects; a++)
    {
        for(int b=0; b<max_objects; b++)
        {
            ...
            if(distance(objects[a].current.x,objects[a].current.y, objects[b].current.x, objects[b].current.y) < objects[a].radius)
            {
                objects[a].within_radius=objects[b].number;
                objects[a].set_color(objects[b].current_color.r, objects[b].current_color.g, objects[b].current_color.b);
