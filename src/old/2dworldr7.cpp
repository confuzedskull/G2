...
//changelog
/*changed object.touching from an array of booleans to an array of integers.
Each index still represents a side of the object, but instead of storing a yes/no it stores the number of the object touching it.*/
//moved the text on the HUD closer together to make room for more lines
//added object.collision() to object.physics() to get rid of a few lines of code in renderscene
//pressing 'i' now toggles HUD on/off
//created more objects to test collision detection
//moved collision_detection around in the renderScene method
//added current_object global variable for switching between different objects
//created init_objects method to initialize objects outside of renderscene
//commented out set_color's in object.collision()
//removed else statements in collision_detection.
//collision detection now works on all objects

//global definitions

int current_object = 1;
const int max_objects = 5;//maximum number of objects to be created for collision testing. must be constant so that array can be made using this number
...
void collision()
{
    ...
    if(touching[2]>0)//touching left
    {
        //set_color(2,1,0);//yellow
    }


    if(touching[3]>0)//touching right
    {
    ...
    if(touching[4]>0)//touching top
    {
        //set_color(1,0.65,0);//orange
    }


    if(touching[5]>0)//touching bottom
    {
        //set_color(1,0,1);//pink
    }

    if(touching[5]==0 && touching[4]==0 && touching[3]==0 && touching[2]==0)
    {
        //undo_color();
    }
...
if (keyStates['w'])
    {
        objects[current_object].current.y+=0.01;
        objects[current_object].action[4]=true;
    }

if (keyStates['s'])
    {
        objects[current_object].current.y-=0.01;
        objects[current_object].action[5]=true;
    }

if (keyStates['a'])
    {
        objects[current_object].current.x-=0.01;
        objects[current_object].action[2]=true;
    }

if (keyStates['d'])
    {
        objects[current_object].current.x+=0.01;
        objects[current_object].action[3]=true;
    }
...
void text ()
{
    ...
    sprintf(text1,"player coordinates=%.3f,%.3f",objects[current_object].current.x,objects[current_object].current.y);
    glutPrint (window_width/-80,window_height/90.5, GLUT_BITMAP_HELVETICA_12, text1, 1.0f,0.0f,0.0f, 0.5f);

    sprintf(text2,"player velocity %.4f,%.4f",objects[current_object].velocity[1].x,objects[current_object].velocity[1].y);
    glutPrint (window_width/-80,3, GLUT_BITMAP_HELVETICA_12, text2, 1.0f,0.0f,0.0f, 0.5f);

    sprintf(text3,"player velocity2 %.4f,%.4f",objects[current_object].velocity[2].x,objects[current_object].velocity[2].y);
    glutPrint (window_width/-80,2.5, GLUT_BITMAP_HELVETICA_12, text3, 1.0f,0.0f,0.0f, 0.5f);

    sprintf(text4,"player resting: %.3f, %.3f",objects[current_object].resting.x,objects[current_object].resting.y);
