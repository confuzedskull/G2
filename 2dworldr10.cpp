...
//changelog
//fixed text display
//started refining classes. working on world class.
//created struct limits
//tidied up code here and there; added comments, deleted old comments etc.
//changed color into a class
//created constant colors
//overloaded color constructor, object constructor,  set_color method, and distance method
//started using object inheritance. created projectile class and player class
...
class object
{
    ...
    void set_boundaries()
    {
        ...
        front.x=current.x+((step[up].x/step_size)*(height/2));
        front.y=current.y+((step[up].y/step_size)*(height/2));
        back.x=current.x+((step[down].x/step_size)*(height/2));
        back.y=current.y+((step[down].y/step_size)*(height/2));
...
class projectile:object
{
    ...
    projectile()
    {
        current.x=objects[current_object].current.x;
        current.y=objects[current_object].current.y;
        width=20;
        height=20;
        current_color=RED;
    }
    ...
    for(int a=0; a<max_objects; a++)
    {
        for(int b=0; b<max_objects; b++)
        {
            if(a!=b)
            {
                ...
                if(distance(objects[a].back.x,objects[a].back.y,objects[b].current.x,objects[b].current.y)<objects[b].radius)
                {
                    objects[a].touching[down]=objects[b].number;
                }

                if(distance(objects[a].front.x, objects[a].front.y,objects[b].current.x,objects[b].current.y)<objects[b].radius)
                {
...
void text ()
{
    ...
    sprintf(text1,"coordinates=%.3f,%.3f", objects[current_object].front.x,objects[current_object].front.y);
